...
const PROJECT_SORT_OPTIONS: Array<{ field: ProjectSortField; labelKey: string }> = [
  { field: "name", labelKey: "pages.projects.sortOptions.name" },
  { field: "updated", labelKey: "pages.projects.sortOptions.updated" },
  { field: "created", labelKey: "pages.projects.sortOptions.created" },
  { field: "targetDate", labelKey: "pages.projects.sortOptions.targetDate" },
];
...
export function Projects() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("app.nav.projects") }]);
  }, [setBreadcrumbs, t]);
  ...
  const sortOption = PROJECT_SORT_OPTIONS.find((option) => option.field === sortField) ?? PROJECT_SORT_OPTIONS[0];
  const sortLabel = t(sortOption.labelKey);

  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={t("pages.projects.selectOrganization")} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="w-fit text-xs" title={t("pages.projects.sort")}>
              <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
              <span>{t("pages.projects.sortLabel", { sort: sortLabel })}</span>
            </Button>
          </PopoverTrigger>
                ...
                >
                  ...
                  {sortField === option.field ? (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {sortDir === "asc" ? t("pages.projects.asc") : t("pages.projects.desc")}
                    </span>
                  ) : null}
                </button>
              ))}
      ...
      {!isLoading && projects.length === 0 && (
        <EmptyState
          icon={Hexagon}
          message={t("pages.projects.empty")}
                        ...
                        trailing={
                          <div className="flex items-center gap-3">
                            <span
                              ...
                              title={t("pages.projects.taskCount", { count: project.taskCount ?? 0 })}
                            >
                              {t("pages.projects.taskCount", { count: project.taskCount ?? 0 })}
                            </span>
                            {project.budget && (
