...
export function Dashboard() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("dashboard.breadcrumbs.dashboard") }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    ...
    return (
      <EmptyState icon={LayoutDashboard} message={t("dashboard.empty.selectOrganization")} />
    );
  }
    ...
    <div className="space-y-6">
      ...
      {pausedBanner?.kind === "imported" ? (
        <InlineBanner
          ...
          icon={PauseCircle}
          title={t(
            pausedImportedCount === 1
              ? "dashboard.banners.importedPausedTitle"
              : "dashboard.banners.importedPausedTitle_plural",
            { count: pausedImportedCount },
          )}
          actions={
            <Button
              size="sm"
            ...
            >
              {resumeImportedAgents.isPending ? t("dashboard.banners.resuming") : t("dashboard.banners.resumeAll")}
            </Button>
          }
        >
          {t("dashboard.banners.importedPausedBody")}
        </InlineBanner>
      ) : pausedBanner?.kind === "all-paused" ? (
        <InlineBanner
          tone="warning"
          icon={PauseCircle}
          title={t("dashboard.banners.allPausedTitle")}
          actions={
            <Button variant="ghost" size="sm" asChild>
              <Link to="/agents">{t("dashboard.banners.reviewAgents")}</Link>
            </Button>
          }
        >
          {t("dashboard.banners.allPausedBody")}
        </InlineBanner>
      ) : null}
      ...
      {hasNoAgents && (
        <div className="flex items-center justify-between gap-3 rounded-md border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-500/25 dark:bg-amber-950/60">
          ...
          >
            {t("dashboard.banners.createOneHere")}
          </button>
        </div>
      )}
        ...
        <>
          {data.budgets.activeIncidents > 0 ? (
            <div className="flex items-start justify-between gap-3 rounded-xl border border-red-500/20 bg-(image:--gradient-extract-1) px-4 py-3">
                  ...
                  <p className="text-sm font-medium text-red-950 dark:text-red-50">
                    {t(
                      data.budgets.activeIncidents === 1
                        ? "dashboard.banners.activeBudgetIncident"
                        : "dashboard.banners.activeBudgetIncident_plural",
                      { count: data.budgets.activeIncidents },
                    )}
                  </p>
                  <p className="text-xs text-red-900/70 dark:text-red-100/70">
                    {t("dashboard.banners.budgetIncidentSummary", {
              ...
              <Link to="/costs" className="text-sm underline underline-offset-2 text-red-900 dark:text-red-100">
                {t("dashboard.banners.openBudgets")}
              </Link>
            </div>
          ) : null}
              ...
              description={
                <span>
                  {data.budgets.pendingApprovals > 0
                    ? t("dashboard.metrics.budgetApprovalsDescription", {
                        count: data.budgets.pendingApprovals,
                      })
                    : t("dashboard.metrics.awaitingBoardReview")}
                </span>
              }
            />
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {SHOW_TASK_PRIORITY_UI && (
              <ChartCard title={t("dashboard.charts.tasksByPriority")} subtitle={t("dashboard.charts.last14Days")}>
                <PriorityChart issues={issues ?? []} />
              </ChartCard>
            )}
            <ChartCard title={t("dashboard.charts.tasksByStatus")} subtitle={t("dashboard.charts.last14Days")}>
            ...
            <div className="min-w-0">
              ...
              {recentIssues.length === 0 ? (
                <Card className="block p-4">
                  <p className="text-sm text-muted-foreground">{t("dashboard.sections.noTasksYet")}</p>
                </Card>
              ) : (
                <Card className="block py-0 divide-y divide-border overflow-hidden">
