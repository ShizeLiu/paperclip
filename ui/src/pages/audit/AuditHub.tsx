...
const AUDIT_SECTION_LABEL_KEYS: Record<AuditSection, string> = {
  activity: "pages.audit.sections.activity",
  runs: "pages.audit.sections.runs",
  costs: "pages.audit.sections.costs",
  budgets: "pages.audit.sections.budgets",
  timeline: "pages.audit.sections.timeline",
};
...
export function AuditHub({ section }: { section: AuditSection }) {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      { label: t("app.nav.audit"), href: section === "activity" ? undefined : "/activity" },
      ...(section === "activity" || !current ? [] : [{ label: t(AUDIT_SECTION_LABEL_KEYS[current.value]) }]),
    ]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={History} message={t("pages.audit.selectOrganization")} />;
  }

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold tracking-tight text-foreground">{t("app.nav.audit")}</h1>
      ...
      >
        <PageTabBar
          items={AUDIT_SECTIONS.map((item) => ({ ...item, label: t(AUDIT_SECTION_LABEL_KEYS[item.value]) }))}
          value={section}
          align="start"
        />
      </Tabs>
