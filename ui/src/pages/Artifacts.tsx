...
export const ARTIFACT_KIND_FILTERS: { value: ArtifactKindFilter; label: string; labelKey: string }[] = [
  { value: "all", label: "All", labelKey: "pages.artifacts.kind.all" },
  { value: "image", label: "Images", labelKey: "pages.artifacts.kind.images" },
  { value: "video", label: "Videos", labelKey: "pages.artifacts.kind.videos" },
  { value: "document", label: "Documents", labelKey: "pages.artifacts.kind.documents" },
  { value: "text", label: "Text", labelKey: "pages.artifacts.kind.text" },
  { value: "file", label: "Files", labelKey: "pages.artifacts.kind.files" },
];

export const ARTIFACT_GROUP_OPTIONS: { value: ArtifactGroupBy; label: string; labelKey: string }[] = [
  { value: "none", label: "None", labelKey: "pages.artifacts.group.none" },
  { value: "task", label: "Task", labelKey: "pages.artifacts.group.task" },
  { value: "parent_task", label: "Parent task", labelKey: "pages.artifacts.group.parentTask" },
];
...
export function Artifacts() {
  ...
  useEffect(() => {
    if (viewingSelectedStack && selectedGroup) {
      setBreadcrumbs([
        { label: t("app.nav.artifacts"), href: "/artifacts" },
        { label: `${selectedGroup.issue.identifier} · ${selectedGroup.title}` },
      ]);
    } else {
      setBreadcrumbs([{ label: t("app.nav.artifacts") }]);
    }
  }, [setBreadcrumbs, viewingSelectedStack, selectedGroup, t]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("pages.artifacts.selectOrganization")} />;
  }
  ...
  const emptyMessage = showGroupCards
    ? searching
      ? t("pages.artifacts.empty.noStacksSearch")
      : t("pages.artifacts.empty.noStacks")
    : searching
      ? t("pages.artifacts.empty.noArtifactsSearch")
      : viewingSelectedStack
        ? t("pages.artifacts.empty.noStackArtifacts")
        : kind === "all"
          ? t("pages.artifacts.empty.noArtifacts")
          : t("pages.artifacts.empty.noArtifactsType");
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            aria-label={t("pages.artifacts.searchAria")}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                aria-label={t("pages.artifacts.groupAria", {
                  group: t(ARTIFACT_GROUP_OPTIONS.find((option) => option.value === groupBy)?.labelKey ?? "pages.artifacts.group.none"),
                })}
