...
const SCOPE_LABEL_KEYS: Record<CompanySearchScope, string> = {
  all: "pages.search.scopes.all",
  issues: "pages.search.scopes.issues",
  comments: "pages.search.scopes.comments",
  documents: "pages.search.scopes.documents",
  artifacts: "pages.search.scopes.artifacts",
  agents: "pages.search.scopes.agents",
  projects: "pages.search.scopes.projects",
};
...
export function Search() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("app.nav.search") }]);
  }, [setBreadcrumbs, t]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-page="search">
      <div className="border-b border-border px-4 py-3 sm:px-6">
        <h1 className="sr-only">{t("app.nav.search")}</h1>
        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-1.5 text-(length:--text-micro) text-muted-foreground">
          ...
          {operatorSuggestions.length > 0 ? (
            <div className="flex flex-wrap items-center gap-1.5" data-testid="search-operator-suggestions">
              {operatorSuggestions.map((suggestion) => (
                <button
                  ...
                  type="button"
                  aria-label={t("pages.search.insertOperatorAria", { token: suggestion.token })}
            ...
            <span className="truncate">
              {t("pages.search.tryPrefix")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">status:todo</code>,{" "}
              <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">assignee:me</code>,{" "}
              {t("pages.search.tryOr")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">updated:&gt;7d</code>.
            </span>
          )}
        </div>
      </div>
          ...
          >
            {scopeValue === scope ? (
              <SearchTabContent
                ...
                scopeLabel={t(SCOPE_LABEL_KEYS[scope])}
              />
            ) : null}
          </TabsContent>
        ))}
    ...
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("pages.search.identifierLookup")}</span> {t("pages.search.identifierLookupBodyPrefix")}{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">PAP-123</code> {t("pages.search.identifierLookupBodySuffix")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("pages.search.quotedPhrases")}</span> {t("pages.search.quotedPhrasesBody")}
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K:</span> {t("pages.search.commandKBody")}
          </li>
        </ul>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? t("pages.search.serverReturned", { status }) : t("pages.search.requestFailed")} {t("pages.search.errorBody")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("pages.search.openTasksFilterView")}
          </Button>
        </div>
      </div>
    );
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("pages.search.noResultsBody", { scope: scopeLabel.toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
          ...
          <Button onClick={openNewIssue} size="sm" variant="default">
            ...
            {t("pages.search.createTaskFromQuery")}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("pages.search.tipFewerTokens")}</li>
          <li>
            {t("pages.search.tipIdentifierPrefix")} <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("pages.search.tipQuotedPhrases")}</li>
        </ul>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>
          {allMatchTotal > totalResults
            ? t("pages.search.resultsOfTotal", { total: totalResults, all: allMatchTotal })
            : totalResults === 1
              ? t("pages.search.oneResult")
              : t("pages.search.resultCount", { count: totalResults })}
          {t("pages.search.sortedBy", { sort: sortLabel })}
          {activeFilterCount > 0
            ? t("pages.search.filtersActive", { count: activeFilterCount })
            : ""}
        </span>
