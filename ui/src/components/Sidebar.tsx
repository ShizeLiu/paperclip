    ...
    >
      ...
      <nav className={primarySidebarStyles.nav}>
        <div className={primarySidebarStyles.group}>
          ...
          {(() => {
            const newTaskButton = (
              <button
                ...
                aria-label={rail ? t("app.nav.newTask") : undefined}
                className={cn(
              ...
              >
                ...
                <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : "truncate"}>{t("app.nav.newTask")}</span>
              </button>
            );
            return rail ? (
              <Tooltip>
                <TooltipTrigger asChild>{newTaskButton}</TooltipTrigger>
                <TooltipContent side="right">{t("app.nav.newTask")}</TooltipContent>
              </Tooltip>
            ) : (
              newTaskButton
            );
          ...
          <SidebarNavItem to="/search" label={t("app.nav.search")} icon={Search} />
          <SidebarNavItem to="/dashboard" label={t("app.nav.dashboard")} icon={LayoutDashboard} liveCount={liveRunCount} />
          <SidebarNavItem
            to="/inbox"
            label={t("app.nav.inbox")}
            icon={Inbox}
            badge={inboxBadge.inbox}
            badgeLabel={t("app.nav.unread")}
          ...
          {showDecisions ? (
            <SidebarNavItem
              to="/decisions"
              label={t("app.nav.decisions")}
              icon={ListChecks}
              badge={attentionCount}
              badgeLabel={t("app.nav.decisions")}
            />
          ) : null}
          {showStatusCards ? (
            <SidebarNavItem to="/status" label={t("app.nav.status")} icon={LayoutGrid} textBadge={t("app.nav.beta")} />
          ) : null}
          {conferenceRoomChatEnabled ? (
            <SidebarNavItem to="/board-chat" label={t("app.nav.conferenceRoom")} icon={MessagesSquare} />
          ) : null}
        </div>

        <SidebarSection label={t("app.nav.work")} collapsible={{ open: workOpen, onOpenChange: setWorkOpen }}>
          <SidebarNavItem to="/issues" label={t("app.nav.tasks")} icon={CircleCheck} />
          {streamlinedUiEnabled ? (
            <>
              <SidebarNavItem to="/projects" label={t("app.nav.projects")} icon={FolderOpen} />
              <SidebarStarredProjects />
            </>
          ) : null}
          <SidebarNavItem to="/routines" label={t("app.nav.routines")} icon={Repeat} />
          <SidebarNavItem to="/artifacts" label={t("app.nav.artifacts")} icon={Package} />
          {showCases ? (
            <SidebarNavItem to="/cases" label={t("app.nav.cases")} icon={Layers} textBadge={t("app.nav.beta")} />
          ) : null}
          {showPipelines ? (
            <SidebarNavItem to="/pipelines" label={t("app.nav.pipelines")} icon={GitBranch} />
          ) : null}
          {showGoalsLink ? (
            <SidebarNavItem to="/goals" label={t("app.nav.goals")} icon={Target} />
          ) : goalsLinkPending ? (
            <div
          ...
          {showWorkspacesLink ? (
            <SidebarNavItem to="/workspaces" label={t("app.nav.workspaces")} icon={GitBranch} />
          ) : null}
          <PluginSlotOutlet
            slotTypes={["sidebar"]}
        ...
        {streamlinedUiEnabled ? (
          ...
          >
            <SidebarNavItem to="/agents" label={t("app.nav.agents")} icon={Users} />
            <SidebarNavItem to="/skills" label={t("app.nav.skills")} icon={Boxes} />
            <SidebarNavItem to="/apps" label={t("app.nav.connectors")} icon={Unplug} />
            <SidebarNavItem to="/activity" label={t("app.nav.audit")} icon={History} />
          </SidebarSection>
        ) : null}
        ...
        {streamlinedUiEnabled ? (
          ...
          <>
            ...
            <SidebarAgents />
            <SidebarSection
              label={t("app.nav.organization")}
              collapsible={{ open: organizationOpen, onOpenChange: setOrganizationOpen }}
            >
              <SidebarNavItem to="/org" label={t("app.nav.org")} icon={Network} />
              <SidebarNavItem to="/apps" label={t("app.nav.connectors")} icon={Unplug} />
              <SidebarNavItem to="/timeline" label={t("app.nav.timeline")} icon={GanttChartSquare} />
              <SidebarNavItem to="/costs" label={t("app.nav.costs")} icon={DollarSign} />
              <SidebarNavItem to="/activity" label={t("app.nav.activity")} icon={History} />
              <SidebarNavItem to="/company/settings" label={t("app.nav.settings")} icon={Settings} />
            </SidebarSection>
          </>
        )}
