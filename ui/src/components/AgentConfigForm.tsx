    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("agentConfig.saving") : t("common.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {forcedKubernetes ? (
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfig.environment")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfig.environment")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentConfig.environmentOverride")}>
              <div className="space-y-2">
                ...
                >
                  <option value="">{t("agentConfig.defaultValue", { value: inheritedEnvironmentLabel })}</option>
              ...
              */}
              ...
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("agentConfig.openCodeDiscoveryLocalOnly", { name: currentDefaultEnvironment.name })}
                </p>
              )}
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label={t("agentConfig.interruptGraceSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel={t("agentConfig.secondsUnit")}
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agentConfig.continueAfterMaxTurnStop")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          {cards ? (
            <h3 className="mb-3 flex items-center gap-2 text-sm font-medium">
              <Bug className="h-3 w-3" /> {t("agentConfig.debugging")}
            </h3>
          ) : (
          ...
          >
            <ToggleField
              label={t("agentConfig.captureRawProviderTraces")}
              hint={t("agentConfig.captureRawProviderTracesHint")}
            ...
            {eff<unknown>("debug", "providerTrace", debug.providerTrace) === "raw" ? (
              <div className="mt-3 flex items-start gap-2 rounded-md border border-border bg-background/60 px-3 py-2 text-xs text-foreground">
                ...
                <span>
                  {t("agentConfig.rawTracingOnDescription")}
                </span>
              </div>
            ) : null}
          </div>
