...
export function MembershipAction({
  ...
  const label = pending
    ? pendingState === "left" ? t("membershipAction.leaving") : t("membershipAction.joining")
    : isLeft ? t("membershipAction.join") : t("membershipAction.leave");
  const ariaLabel = isLeft
    ? t("membershipAction.joinResource", { name: resourceName })
    : t("membershipAction.leaveResource", { name: resourceName });
